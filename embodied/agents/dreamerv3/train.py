...
def main(argv=None):
    ...
    if args.script == "train":
        ...
        raise NotImplementedError(
            f"Only run.script=train and run.script=eval_only are kept in this repo, got: {args.script}"
        )
...
def make_env(config, index, **overrides):
    ...
    if suite != "humanoid":
        raise NotImplementedError(
            f"Only the humanoid suite is kept in this research repo, got: {suite}"
        )
    module = importlib.import_module("embodied.envs.from_gymnasium")
    ctor = getattr(module, "FromGymnasium")
    kwargs = config.env.get("humanoid", {})
