...
class ObservationWrapper(BaseWrapper):
    def __init__(self, task, **kwargs):
        ...
        self._tactile_concat = str(kwargs.get("tactile_concat", "false")).lower() == "true"
    ...
    def _apply_proprio_noise(self, x):
        if self.proprio_noise <= 0:
            return x
        return x + np.random.normal(0.0, self.proprio_noise, size=x.shape).astype(x.dtype)

    def _apply_tactile_dropout(self, x):
        p = self.tactile_dropout
        if p <= 0:
            return x
    ...
    def observation_space(self):
        ...
        if self._tactile_ob:
            ...
            if self._tactile_concat and "tactile" in tactile_example:
                tactile_spaces = [(
                    "tactile",
    ...
    def get_obs(self):
        ...
        state = self._apply_proprio_noise(state)
        ...
        if self._tactile_ob:
            ...
            # If tactile_concat is enabled, tactile_obs returns {'tactile': vec}
    ...
    def get_tactile_obs(self):
        ...
        if self._tactile_flat:
            for k in list(touch.keys()):
                touch[k] = touch[k].astype(np.float64).reshape(-1)
        if self._tactile_concat:
